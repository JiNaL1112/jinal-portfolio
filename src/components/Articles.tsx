...
// Redesigned with card layout — image + tag + date + title + CTA

import { Link } from 'react-router-dom';
import { articles } from '../data/articles';
import styles from './Articles.module.css';

// SVG icon fallbacks per tag category
function ArticleIcon({ tags }: { tags: string[] }) {
  const tag = tags[0]?.toLowerCase() ?? '';
  if (tag.includes('kubernetes') || tag.includes('k8s')) {
    return (
        ...
        {featured && (
          ...
          >
            ...
            <div
              ...
              style={{ background: featured.image ? undefined : 'linear-gradient(135deg,#0D1117 0%,#111827 100%)' }}
            >
              {featured.image ? (
                <img
                  src={featured.image}
